...
\newshadetheorem{exdef}{Excursion Definition}
...
& = \l(|e(t_0,\Delta t)| + \sum_{k=0}^{i }\einsdurch{1-\Delta t L}\eta(\Delta t)\r) e^{\sum_{k=0}^{i }\frac{2\Delta t L }{1-\Delta t L}}. \tag{$\star$}
\end{align*}
...
\end{align*}
Inserting this into ($\star$) gives the result.
\end{proof}

\begin{theorem}
If a one-step method with Lipschitz condinitions (II.3) and (II.4) is consistent of order $p\in\N$ for an ODE (II.1) and if the initial value $y_0$ meets
\[
...
\]
as an approximation of $y(\xi_i)$, where $i=1,\dots,s$. This is called the \textit{Runge-Kutta methods}.
...
\end{align*}
{\rot \texttt{The drawing is missing.}} A Runge-Kutta method hence allows us to approximate $f$ on intermediate steps. This can be very useful, since in many cases the function $f$ is hard or expensive to evaluate.
\end{example}


\noindent Reconsider the (maybe non-linear) system (II.$\star$) for computing the gradients $r_j$. The following theorem shows that the system can be solved if $f$ satisfies certain conditions. In particular, we can, in some sense, buy the solveability of the system (II.$\star$) by choosing smaller time steps.
...
\begin{enumerate}[(a)]
\item The method has at least order of consistency at least $p=1$ if
\[
\sum_{j=1}^{s }b_j = 1 \und \sum_{\nu=1 }^{s }a_{j\nu} = c_j \tag{II.7}
\]
holds for all $j=1,\dots,s$.
\item The method has order of consistency at least $p=2$ if it holds (II.7) and
\[
\sum_{j=1}^{s }b_j c_j = \einhalb \tag{II.8}
\]
holds.
\item The method has order of consistency at least $p=3$ if (II.7), (II.8) and
\[
...
\end{theorem}
\begin{proof}
The proof is an exercise on the fourth sheet.
\end{proof}
...
We are heading towards the implicit Runge-Kutta methods and try to motivate it.
...
we obtain
\[
u_s(t_i+c_j\Delta t) = u_s(t_i) + \Delta t \sum_{\ell=1}^{s }a_{j\ell} f\Big(t_i+c_j\Delta t,~u_s(t_i+c_j\Delta t)\Big),\tag{$\star$}
\]
or
\[
u_s(t_i+\Delta t) = u_s(t_i) + \Delta t \sum_{j=1}^{s }b_j f\Big(t_i+c_j\Delta t,~u_s(t_i+c_j\Delta t)\Big). \tag{$\star\star$}
\]
Since in the collocation method we set $u_s(t_i)= y_i$ and $u_s(t_{i+1})=y_{i+1}$, we have the Runge-Kutta method in ($\star$) and ($\star\star$).
\end{proof}
